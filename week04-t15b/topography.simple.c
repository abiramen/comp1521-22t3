...
main__for_points_init:
    int i = 0;

main__for_points_cond:
    if (i >= N_POINTS) goto main__for_points_end;

main__for_points_body:

    // &my_points[i] = my_points + i * 8

    int row = my_points[i].row; // &my_points[i] + 0
    int col = my_points[i].col; // &my_points[i] + 4
    int height = topography_grid[row][col]; // &topography_grid[row][col] = topography_grid + (row * MAP_SIZE + col) * sizeof(int)
    printf("Height at %d,%d=%d\n", row, col, height);
main__for_points_step:
    i++;
    goto main__for_points_cond;
main__for_points_end:
    return 0;
}
